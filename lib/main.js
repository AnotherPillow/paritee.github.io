...
function openTab(event) {
	...
	for (i = 0; i < path.length; i++) {
		if (tabName = path[i].getAttribute('data-target'))
			break;
	}
...
document.addEventListener('DOMContentLoaded', function () {
	...
	Array.from(document.querySelectorAll('.field-body')).filter(x=>x.children.length>3).forEach(x=>x.classList.add('has-four-children'))
});
